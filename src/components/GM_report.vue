...
设模型中输入的参数为$ X_1^0 $、$X_2^0 $、$X_3^0 $，根据上面的1-AGO操作，生成$ X_1^1 $、$X_2^1 $、$X_3^1 $
列出灰度微分方程如下：
$$\\frac{\\text{d}X_1^1(rp+t)}{\\text{d}t} +b_1X_1^1(rp+t) = b_2X_2^1(t) + b_3X_3^1(t) $$
将微分方程进行移项，并将微分项看为$ X_1^1(rp+t+1)$ 和$ X_1^1(rp+t)$  的平均值，改写成矩阵乘法的形式，有：
![图片alt](矩阵乘法.jpg "图片title")
等式右边的矩阵和列向量分别记为$B $和$Y_R$ ，根据最小二乘法，有：
$$ \\begin{bmatrix}
    b_1&b_2&b_3
\end{bmatrix}^T = (B^TB)^{-1} Y_R $$
再根据微分方程的解的形式，有：
![图片alt](微分方程.jpg "图片title")
